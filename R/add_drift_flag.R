...
add_drift_flag <- function(df) {
  ...
  progressive_drift <- function(x) {
    ...
    model <- stats::lm(x ~ seq_along(x), na.action = stats::na.omit)
  ...
  new <- df %>%
    ...
    dplyr::mutate(visit_group = cumsum(stringr::str_detect(tidyr::replace_na(flag, ""), stringr::regex(visit_regex, ignore_case = TRUE)))) %>%
    ...
    dplyr::mutate(
      ...
      r2_s_right = slider::slide_index_dbl(.x = mean, .i = DT_round, .f = progressive_drift, .before = lubridate::minutes(1440)),
      r2_s_center = slider::slide_index_dbl(.x = mean, .i = DT_round, .f = progressive_drift, .before = lubridate::minutes(720), .after = lubridate::minutes(720)),
      # 3-day windows with different alignments
      r2_l_right = slider::slide_index_dbl(.x = mean, .i = DT_round, .f = progressive_drift, .before = lubridate::minutes(4320)),
      r2_l_center = slider::slide_index_dbl(.x = mean, .i = DT_round, .f = progressive_drift, .before = lubridate::minutes(2160), .after = lubridate::minutes(2160)),
      ...
      failed = slider::slide_index_dbl(.x = tightest_r, .i = DT_round, .before = lubridate::minutes(1440), .f = too_steady)
    ) %>%
    data.table::data.table() %>%
    # Add drift flag for periods with consistent linear trends
    add_flag(failed == 1, "drift") %>%
    dplyr::select(-visit_group, -r2_s_right, -r2_s_center, -r2_l_right, -r2_l_center, -tightest_r, -failed)
