...
cal_exp_one_point_drift <- function(df, lm_trans_col, pre_col, post_col, wt_col,
  ...
  if (drift_type == "exponential") {
    df <- df %>%
      mutate(wt_exp = (exp(.data[[wt_col]]) - 1) / (exp(1) - 1))
  ...
  if (correction_type == "multiplicative") {
    ...
    df <- df %>%
      mutate(!!transformed_col := .data[[lm_trans_col]] * (1 - .data[[wt_col_to_use]] + .data[[wt_col_to_use]] * drift_ratio))
    ...
    df <- df %>%
      mutate(!!transformed_col := .data[[lm_trans_col]] + (.data[[wt_col_to_use]] * standard_delta))
  }
